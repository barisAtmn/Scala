...
object JSONSerialization extends App {
  ...
  case class Baris(name:String, feeds:List[Feed], users:List[User])
  ...
  implicit object barisConverter extends JSONConverter[Baris] {
    def convert(baris: Baris): JSONValue = JSONObject(Map(
      "user" -> baris.name.toJSON,
      "feeds" -> JSONArray(baris.feeds.map(_.toJSON)),
      "posts" -> JSONArray(baris.users.map(_.toJSON))
    ))
  }
  ...
  val bar = Baris("John", List(feed,feed2), List(john))
